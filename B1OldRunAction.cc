...
B1OldRunAction::B1OldRunAction(G4bool headless)
  : G4UserRunAction(),
    fHeadless(true),
    fEdep(0.),
    fEdep2(0.),
...
B1OldRunAction::~B1OldRunAction()
{}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

void B1OldRunAction::BeginOfRunAction(const G4Run* run)
{ 
...
void B1OldRunAction::EndOfRunAction(const G4Run* run)
{
  ...
  if (fHeadless) {
    ...
    */
    ...
    plotBeginningPosition->SetTitle("");
...
void B1OldRunAction::AddEdep(G4double edep)
{
  fEdep  += edep;
  fEdep2 += edep*edep;
...
void B1OldRunAction::AddEdepByProcess(std::map<G4String, G4double> edepByProcess) {
...
void B1OldRunAction::AddDepositCount(std::map<G4String, G4int> depositCount) {
...
void B1OldRunAction::AddDeviationAngle(G4double deviationAngle)
{
  if (deviationAngle != -10.) fDeviationAngleVector.push_back(deviationAngle);
}

void B1OldRunAction::AddBeginningPosition (std::tuple<G4double, G4double> beginningPosition) {
  fBeginningPositionVector.push_back(beginningPosition);
}

void B1OldRunAction::AddEndPosition (std::tuple<G4double, G4double> endPosition) {
...
void B1OldRunAction::AddQuartzWindow1Edep (G4double edep) {
  fQuartzWindow1EdepVector.push_back(edep);
}

void B1OldRunAction::AddCherenkovEndpointVector (std::vector<G4ThreeVector> cherenkovEndpoints) {
  for (auto endpoint: cherenkovEndpoints) fCherenkovEndpointVector.push_back(endpoint);
}

void B1OldRunAction::AddCherenkovArrivalTime(G4double arrivalTime) {
  fCherenkovArrivalTimes.push_back(arrivalTime);
}

void B1OldRunAction::AddCherenkovCount(G4int cherenkovCount) {
